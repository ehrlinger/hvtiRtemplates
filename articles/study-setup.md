# Set up an hvtiR study for descriptive and EDA work

This is the common first tutorial for an hvtiR study. Start on one of
two paths:

1.  create a study that has no files yet; or
2.  adopt an existing study without rewriting its working directories.

Both paths meet at the same study contract. From there, register the
data, write an `eda` analysis set, and create the five jobs used before
a specific analysis begins: general checks, descriptive tables, goodness
of follow-up, trends, and EDA postage stamps.

The executable example builds both paths from 40 synthetic observations
and a named 24-row subset. Every file stays in a temporary directory.
During rendering, that directory is removed after the R chunks run; the
displayed figure is embedded before cleanup. Run the chunks in order in
one interactive session to inspect the files yourself. For the
statistical mapping behind the jobs, see [From SAS descriptive jobs to
...
A new study begins at a path that does not exist. `study_setup()`
creates the numbered working directories, `_study.yml`, an R project,
`.Renviron`, and `.renvignore`. Use the study’s real Study Tracker ID
and title.
...
#> Study: /tmp/RtmpK1toKf/file1d653b6086fe/new-study
#> 
...
#> [1] "00_datasets"      "10_descriptive"   "20_distributions" "30_analyses"     
#> [5] "40_graphs"        "50_documents"     "90_estimates"
```

The data build comes next. Put its outputs in `00_datasets/`, then
register the default study dataset. Register a distinctly named dataset
only when a job must read a second file as it stands.
...
An adopted study keeps its bare working-directory names and all
study-authored programs, logs, listings, and output. Before adoption,
inventory two pieces of old scaffolding that should not become part of
the study contract:

- a local `.git` repository left by the former setup process; and
- obsolete copied template files beginning with `tp` anywhere in the
  study working directories.

Do not remove either by a broad wildcard. Confirm the study root,
inspect the exact matches, and choose a backup outside that root. Moving
them makes the cleanup recoverable. A real study needs a second person
to confirm the inventory and backup destination before the move.

``` r

normalizePath(adopted_root)
#> [1] "/tmp/RtmpK1toKf/file1d653b6086fe/legacy-study"
...
legacy_git
#> [1] "/tmp/RtmpK1toKf/file1d653b6086fe/legacy-study/.git"
legacy_tp
#> [1] "/tmp/RtmpK1toKf/file1d653b6086fe/legacy-study/tp.shared.sas"
...
`adopt = TRUE` is additive. It records identity and environment files
while preserving the legacy layout. Recovery of missing study state is a
separate operation.
...
#> Study: /tmp/RtmpK1toKf/file1d653b6086fe/legacy-study
#> 
...
`study_setup()` reports a missing `renv.lock` until the study
initializes its package environment. Do this in the real study root for
either path; it is not executed in this temporary example because
`renv::init()` changes the active R project. Snapshot again after
installing the packages used by the study and whenever an accepted
...
From this point the same calls work with either path. This example
continues with the adopted study so the next section can demonstrate
migration from SAS. Registration records the checksum and cohort counts
for each dataset. The study cohort has 40 observations; the named subset
has 24.
...
dataset. Add its definition under `analysis_sets:` in `_study.yml`. Name
the patient identifier, the columns to keep, ordered exclusions, and
expected counts. This synthetic set keeps every row:
...
Write the set after its declaration. `write_analysis_set()` records its
link to the registered dataset and stops later reads if either the
source data or the declaration changes. The helper below preserves other
analysis sets and refuses to replace an existing `eda` definition;
review and rename an existing set deliberately instead of overwriting
it.
...
These five jobs complete the descriptive and EDA stage.
[`open_job()`](https://ehrlinger.github.io/hvtiRtemplates/reference/open_job.md)
is the interactive entry point: it creates a missing job and opens it,
or opens an existing job without overwriting edits. The executable
example uses
[`add_job()`](https://ehrlinger.github.io/hvtiRtemplates/reference/add_job.md)
so rendering this article does not open five editor windows.
...
new_jobs
#>                                                                               general 
#> "/tmp/RtmpK1toKf/file1d653b6086fe/new-study/10_descriptive/cohort-eda-dc-general.qmd" 
#>                                                                                tables 
#>  "/tmp/RtmpK1toKf/file1d653b6086fe/new-study/10_descriptive/cohort-eda-dc-tables.qmd" 
#>                                                                                  gfup 
#>    "/tmp/RtmpK1toKf/file1d653b6086fe/new-study/10_descriptive/cohort-eda-dc-gfup.qmd" 
#>                                                                                trends 
#>       "/tmp/RtmpK1toKf/file1d653b6086fe/new-study/40_graphs/cohort-eda-dp-trends.qmd" 
#>                                                                               postage 
#> "/tmp/RtmpK1toKf/file1d653b6086fe/new-study/10_descriptive/cohort-eda-dp-postage.qmd"
```

In a real study, use
[`open_job()`](https://ehrlinger.github.io/hvtiRtemplates/reference/open_job.md)
for each row, work its `EDIT:` markers from top to bottom, and render
drafts as you go. Use `render_job(job, final = TRUE)` only after every
marker is resolved. The descriptive, follow-up, and postage jobs can
read the `eda` analysis set. The trends job reads a registered dataset
directly. Choose the whole study dataset or a named dataset only when it
is the intended population.

#### When registered data change

Do not overwrite a registered dataset and run `update_manifest()` alone.
Registration also records cohort metadata in `_study.yml`, so that
shortcut can leave the two authoritative files disagreeing. Keep the
existing extract unchanged until `hvtiRutilities` provides a coordinated
update operation, then re-write every dependent analysis set and
re-render these jobs.
...
`dc-general` template. Scaffold that job directly, then review its
editable choices against the registered data and protocol.
