...
2.  Work the `EDIT:` markers, rendering from the study root with
    `quarto render 10_descriptive/cohort-eda-dc-tables.qmd` as you go.
    Unfinished work renders as a draft, as far as the markers you have
    worked.
3.  Stop when every marker is gone and the draft banner with it.
...
Every line a study author has to change carries an `EDIT:` marker.
Markers left in a job do not stop its render; while any remain, the
...
The banner does not mean every section will run. A section whose marker
is not yet worked still reads the template’s placeholders, and most of
those stop with their own error rather than draw a table from nothing. A
new `dc-tables` job, for instance, stops with “Unknown table
variable(s)” until `GROUPS` names your columns, and before that if the
`eda` analysis set has not been written. Work the markers from the top
of the file down, and each render gets further.
...
comment, counts as much as one in a code chunk. When the last marker
goes, so does the banner.

For a final render, such as the one that goes to an investigator, make
an unfinished job stop rather than render as a draft:
