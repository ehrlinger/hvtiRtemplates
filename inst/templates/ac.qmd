...
knitr::kable(
  do.call(rbind, lapply(names(DERIVED), function(cat) {
    src <- DERIVED[[cat]]
...
km_by <- function(dd, var) {
  ...
  src <- rbind(
    if (length(imp_lv)) data.frame(
      source = imp_lv, n = vapply(imp_lv, function(l) sum(raw == l, na.rm = TRUE), integer(1)),
      reason = "mean-imputed level, not an observed value"),
    if (anyNA(raw)) data.frame(
      source = "NA", n = sum(is.na(raw)), reason = "missing outright")
  )
...
report_unknown <- function(fit, var) {
  ...
  print(knitr::kable(src, row.names = FALSE, caption = paste0(
