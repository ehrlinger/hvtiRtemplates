...
# Each name is also a figure's filename, so a name used twice, within a list
# or across both, would overwrite the first figure without a word.
.dup <- unique(c(names(PANELS), names(EVENTS))[duplicated(c(names(PANELS), names(EVENTS)))])
if (length(.dup)) {
