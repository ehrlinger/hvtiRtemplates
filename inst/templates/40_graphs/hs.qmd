...
```{r}
#| label: study-choices
# Follow-up and event columns, expected counts, and prediction horizons.
# EDIT: time, event, expected counts, and prediction horizons.
TIME     <- "iu_dead"
...
# EDIT: population survival matching and life table settings.
AGE_COL   <- "age"
...
<!-- EDIT: apply the same filter, event, time, and expected counts as the `hm`
     job. The rows this job predicts for MUST be the rows the model was fitted
     on, or the predictions describe a cohort the model never saw. That failure
     is silent: `predict()` returns a number for any row you hand it, whatever
     cohort it came from. The invalid expected counts make an unedited job stop
     before it can predict for an unreconciled cohort. -->

```{r}
#| label: cohort
d <- read_built()
# EDIT: apply the same job filter as hm, before both the observed counts and gate.
#
