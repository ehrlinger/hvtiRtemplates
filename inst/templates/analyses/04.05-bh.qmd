...
#
# NOTE: this table's at-least-one figure is `pct_any`, not the `union_pct` name
# the earlier version of this chunk used. Every value is unchanged; only the
# name is, because `boot_clusters()` has called the same quantity `pct_any`
# since 0.1.1 and two names for one number across two functions in one report
# is exactly the drift this extraction removes.
