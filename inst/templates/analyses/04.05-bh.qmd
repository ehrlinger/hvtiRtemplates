...
# The nested fields are checked by name too. `boot` being present says nothing
# about what is inside it, and every table below reaches into it -- a missing
# `summary` surfaces thirty lines later as a NULL subscript, naming nothing.
if (!length(.absent)) {
  # Subset AFTER paste0, never inside it: paste0("boot$", character(0)) is
  # "boot$", not character(0), so pasting the filtered vector reports a missing
  # field named nothing on every healthy run.
  .nested <- c("replicates", "summary", "n_success", "n_failed")
  .absent <- paste0("boot$", .nested)[!.nested %in% names(bag$boot)]
}
if (length(.absent)) {
