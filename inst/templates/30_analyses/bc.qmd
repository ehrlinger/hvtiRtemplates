...
# External producers that cannot attach hvti_provenance must put their original
# provenance_data() records here. Leave empty for package-produced bags.
BOOTSTRAP_DATA <- list()
...
.bag_read <- hvtiRtemplates:::.read_bootstrap_artifact(
  .single_file, "bootstrap-bag", .cfg, BOOTSTRAP_DATA
)
bag <- .bag_read$value
...
report <- hvtiRtemplates:::.attach_handoff_lineage(
  ...
  analysis = .bootstrap_lineage$analysis, cohort = .bootstrap_lineage$cohort
)
...
cat(hvtiRtemplates:::.embed_provenance(
  ...
  extra = c(
    list(subject = SUBJECT, type = TYPE),
    if (is.null(.bootstrap_lineage$analysis)) list() else list(analysis = .bootstrap_lineage$analysis),
    if (is.null(.bootstrap_lineage$cohort)) list() else list(cohort = .bootstrap_lineage$cohort)
  )
))
```
