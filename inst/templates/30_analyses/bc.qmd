---
title: "Bootstrap variable selection: Cox"
...
pool, `sle=`, `sls=`, `resampl=` and `proc=phreg`.

This job reports a bootstrap variable selection screen over a **Cox
proportional hazards** model: how often each candidate survived stepwise selection across resamples,
...
A companion runner screens.

⚠️ **Whatever job fits your final model is a parallel analysis, not a
downstream step.** The taxonomy has no Cox-model prefix -- `cm` is Cox
*matching* -- so this template names no companion rather than inventing one.
Do not describe this job's retained set as what some later model fits, and do
not write a handoff file for it: an earlier study report did both for the
hazard pair, nothing read the file, and the claim was not true. A reader who
believes it will assume the fitted model was screened for reliability first.
...
# TemporalHazard is deliberately absent: a Cox screen never touches it. The
# runner calls hvtiRbootstrap::boot_select() with fit_cox() and converts the
# result with boot_bag(). survival is the study's dependency, loaded by the
# runner; this report reads the bag and never fits anything.
...
forced into every replicate.** `boot_bag()` refuses a `base_params` naming a
term the screen does not carry, which is exactly what `"(Intercept)"` copied
from a logistic runner does here — and that refusal is correct: every frequency
below excludes the base model, so a name matching nothing would exclude nothing
and report the base model as a candidate.

If your screen forces nothing, there is no free base parameter and the
standard-deviation check below reports `NA` rather than passing. **That check
is then not running.** It is the one that catches a bootstrap which refit
nothing and reported every replicate a success, so a screen whose frequencies
are all 100% would render green with nothing to stop it. Force at least one
term, or read the frequency table knowing the guard is inactive.
...
# A Cox screen has no phases: a candidate is offered once, so one term is one
# screening decision and there is nothing to split a term name on. A
# multiphase hazard screen is a different job -- that is `bh`, which passes a
# term-splitting rule to the same functions.
#
...
# statement about your candidate pool, not about proportional hazards.
#
...
# retained set written for a model job to read. The study report that wrote
# such a file for the hazard pair also claimed the model fits what the screen
# retained. Nothing read the file, and the claim was not true. A file whose existence implies a dependency that does not exist
