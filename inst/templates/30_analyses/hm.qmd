...
```{r}
#| label: study-choices
# Follow-up time, event, and SAS source for covariates.
# EDIT: time and event variables.
TIME   <- "iu_dead"

EVENT  <- "idead"

# EDIT: SAS job and macro holding covariate blocks.
SAS_JOB   <- c("analyses", "hm.dead.sas")

SAS_MACRO <- "^\\s*%macro\\s+model\\s*;"

# Phase shape and scale parameters.
# EDIT: phase shape and scale parameter names.
...
# EDIT: selection settings from the SAS job.
SLENTRY   <- 0.1     # SAS sle=
