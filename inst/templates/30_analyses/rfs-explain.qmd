---
title: "Random forest survival: explain"
...
.forest_time <- forest$yvar.names[[1L]]
.forest_status <- forest$yvar.names[[2L]]
.forest_event <- forest$yvar[[.forest_status]]
.forest_cohort <- list(
  n = as.integer(nrow(forest$yvar)),
  n_events = as.integer(sum(.forest_event == 1)),
  n_censored = as.integer(sum(.forest_event == 0))
)
hvtiRutilities::record_provenance(
  ...
  extra = list(
    ...
    type = TYPE,
    analysis = list(
      time = list(variable = .forest_time),
      event = list(variable = .forest_status, event = 1L, censored = 0L)
    ),
    cohort = .forest_cohort
  )
)
```
