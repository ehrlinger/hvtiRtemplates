...
<!-- EDIT: name the job this replaces (an `rfsrc.*` or `rf.*` job with a
     continuous outcome), and say which outcome and candidate predictors it
     used. -->
...
# tree count, whose error curve and predicted-against-observed plot look
...
# `rfr-fit` writes the forest through this and `rfr-explain` reads it back,
...
CACHE_DIR <- dirname(set_path("estimates", "rfr.rds"))
...
```{r}
#| label: save
# The product of this job, and what rfr-explain reads. It is separate from the
...
saveRDS(forest, set_path("estimates", "rfr.rds"))
```
