---
title: "Bootstrap variable selection: linear"
...
pool, `sle=`, `sls=`, `resampl=` and `proc=reg`.

This job reports a bootstrap variable selection screen over a **linear**
...
A companion runner screens; the companion `rm` job fits the multivariable
model.

⚠️ `br` and `rm` are **parallel analyses, not a pipeline.** `%bootreg` and the
model job were parallel in SAS and these jobs reproduce that. Do not describe
this job's retained set as what `rm` fits, and do not write a handoff file for
...
# TemporalHazard is deliberately absent: a linear screen never touches it.
# The runner calls hvtiRbootstrap::boot_select() with fit_linear() and
...
responds to residual variance in a way a binary outcome's does not. Two runs of
the same pool against the same cohort, one on a raw outcome and one on a
...
# A linear screen has no phases: a candidate is offered once, so one term is
...
# statement about your candidate pool, not about linear regression.
#
...
# retained set written for `rm` to read. `br` and `rm` are parallel analyses,
...
report_file <- set_path("estimates", "br-report.rds")
