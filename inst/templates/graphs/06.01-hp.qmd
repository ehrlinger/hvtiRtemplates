...
t_plot <- exp(seq(-8, log(max(years)), length.out = 1000))
```
...
knitr::kable(data.frame(
  max_grid_point   = max(years),
  end_of_follow_up = maxfup,
  extrapolating    = max(years) > maxfup
))
...
if (max(years) > maxfup) {
  warning("The grid extends to ", format(max(years), digits = 4),
