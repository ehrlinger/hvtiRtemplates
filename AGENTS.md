...
Eight exports across eleven source files: `hvti_taxonomy()`, `hvti_non_prefixes()`,
`template_list()`, `template_path()`, `add_job()`, `open_job()`, `render_job()` and `migrate_job()`.
...
| workflow | fails on |
|---|---|
| `R-CMD-check.yaml` | `R CMD check` across platforms. The template catalog ships in the tarball, so the catalog-reading tests run on **every** leg: expect `SKIP 0` on macOS and Ubuntu and `SKIP 1` on Windows, the one being `test-add-job.R`'s deliberate `skip_on_os("windows")` (POSIX mode bits do not govern writability there). Any other skip is a gate that stopped running: from #138 (2026-09-21) until #150, two `test-vignette-migration.R` tests read `vignettes/` with no fallback to the installed `doc/`, so every leg ran two skips high and nobody noticed. A vignette test must read `system.file("doc", ...)` when the checkout is absent. Expect `WARN 0`: the rfr-explain test muffles varPro 3.3.0's notice that it drops rows with missing values, and only that notice, so any warning in the summary is new |
...
⚠️ **`check-manual.yaml` is not a PR gate.** Its triggers are `push` to `main` and
`workflow_dispatch`; there is no `pull_request` among them, so the PDF manual build
