...
- **`main` is protected by a GitHub ruleset, and nothing in this repo records that.** A clone
  ...
  **automatic Copilot code review**. A direct push to `main` is rejected by the server —
  ...
  The fix is to open against `main`. If you have already stacked, request Copilot by hand
  from the Reviewers menu — the REST `requested_reviewers` endpoint returns 200 and silently
  does nothing for the reviewer bot, so it cannot be scripted that way.
