...
- **`main` is protected by a GitHub ruleset, and nothing in this repo
  ...
  **The timeline resolves “not requested” from “slow”, which the reviews
  ...
  their own PR, the diff can go unread by anything that reads for
  correctness; CI checks compilation, tests, lint and the count guards,
  and none of those do. ⚠️ **Run `/code-review` locally before opening a
  PR**, and say in the PR body that it stood in for the bot, so a reader
  can tell a reviewed change from an unreviewed one. Do it for every
  substantive push, not only the first, because the bot will not.
  Measured on
