...
| workflow | fails on |
|---|---|
| `R-CMD-check.yaml` | `R CMD check` across platforms. The template catalog ships in the tarball, so the catalog-reading tests run on **every** leg: expect `SKIP 0` on macOS and Ubuntu and `SKIP 1` on Windows, the one being `test-add-job.R`'s deliberate `skip_on_os("windows")` (POSIX mode bits do not govern writability there). Any other skip is a gate that stopped running: from #138 (2026-09-21) until the fix for it, two `test-vignette-migration.R` tests read `vignettes/` with no fallback to the installed `doc/`, so every leg ran one skip high and nobody noticed. A vignette test must read `system.file("doc", ...)` when the checkout is absent. Expect `WARN 1` on every leg: `test-rf-templates.R`'s rfr-explain test, where `varpro()` reports the observations it drops for missing values |
