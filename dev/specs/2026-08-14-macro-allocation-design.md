...
✅ **ERRATUM 2026-09-09, resolved the same day.** Until then the scan globbed
`macro.library/*.sas` and read **176 of the library's 310** top-level source
files — 57% — never opening 105 extensionless files or 29 dot-named ones. The
picker is now a denylist and every table below is re-rendered from the re-run
map. What changed: 176 → **310** files, 94 → **119** allocated, 73 → **178**
corpus-only. **No pre-existing file changed tier or destination**, so the
allocation was short rather than wrong. `kaplan`, which the call-site scan
validated as a shared house primitive without being able to open it, now
allocates to `hvtiRutilities`.

🔴 **One consequence is not resolved:** the newly-visible files close a
`hvtiRutilities -> TemporalHazard` cycle at the package level. See
*Cross-package dependencies* below, and
...
| `hvtiRbootstrap` | 31 |
| `TemporalHazard` | 18 |
| `hvtiRutilities` (shared) | 33 |
| `hvtiRdatabuild` | 8 |
| `hvtiPlotR` | 7 |
| `hvtiRlifetables` (override) | 5 |
| **Allocated** | **119** |
| Travels with a dependent | 5 |
| Blocked on an unowned prefix | 8 |
| Corpus-only | 178 |
| **Total** | **310** |
...
`CR_CIF_CP_variance.sas`, `bootstrap-test.models_wts.sas`, `bootstrap.models_wts.sas`, `chisqgf`, `chisqgf.exact`, `cindex_hazard.sas`, `cindex_hazard_tvc.sas`, `haz_to_mi.sas`, `hazplot.pc`, `hazplot.production`, `hazplot.sas`, `hazplot02252013.sas`, `hazplot10242003.sas`, `hazplot_cltest.sas`, `markov.sas`, `mi_to_haz.sas`, `nelsont`, `nelsont.sas`

### `hvtiRutilities` - shared (33)

`bootstrap.summary.sas`, `cumdist`, `cumdist.test`, `deciles`, `deciles.hazard`, `deciles.hazard.test`, `deciles.joan`, `deciles.new`, `decomposition.sas`, `decomposition_JR.sas`, `kaplan`, `kaplan.int`, `kaplan.int.sas`, `kaplan_jr.sas`, `kaplanol`, `kaptest`, `nelsonl`, `nelsonl.sas`, `plot`, `plot.compile`, `plot.sas`, `plot_8.sas`, `plot_emf.sas`, `plotjoan.sas`, `repeat.sas`, `repeat.testmacro.sas`, `repeated.sas`, `stddiff.sas`, `sumtest`, `vartest`, `vartest2`, `vartest3`, `vartest4`
...
`logistc`, `logistc.sas`, `logistc_p.sas`, `logistc_sts.sas`, `logit_nominal.sas`, `logit_ord.sas`, `mxpredc.sas`
...
from another package. **88** file-level dependencies span **6** package pairs.
🔴 **The package graph is no longer acyclic**; the file graph still is:

```
TemporalHazard  -> hvtiRutilities  (44)
hvtiRbootstrap  -> hvtiRutilities  (14)
hvtiRbootstrap  -> hvtiPlotR       (13)
hvtiRutilities  -> TemporalHazard  (8)
hvtiRdatabuild  -> hvtiRutilities  (6)
hvtiRtables     -> hvtiRutilities  (3)
```
...
The **acyclic** half was true throughout the period described above, and that
was the part worth noting: a stale number sitting beside a correct claim is
harder to see than a wrong one on its own. It is verified rather than asserted,
by a depth-first colouring in the scan, at package level and at file level.

🔴 **It stopped being true on 2026-09-09, and the check is what said so.** Fixing
the library picker admitted 134 files the scan had never opened, and among them
a `hvtiRutilities -> TemporalHazard` edge that closes a cycle against the 44
edges running the other way. Every one of the eight edges is the same pair of
calls:
...
The calibration family votes to `hvtiRutilities` on two or more owned prefixes;
`chisqgf*` votes to `TemporalHazard` on `hz`/`hs`; and calibration calls
goodness-of-fit. Both votes are correct applications of the rule, which is what
makes this a **design** problem rather than a scan defect — the rule has no way
to express that one of its outputs is the package the other depends on.

⚠️ `hvtiRutilities` is the base of the family. It cannot depend on
`TemporalHazard`, so one of the two votes has to lose: either `chisqgf*` is a
shared primitive that belongs in `hvtiRutilities`, or the `deciles.*`
calibration family belongs in `TemporalHazard` beside the fit it calibrates.
**Undecided.** Until it is decided, this document states a cycle rather than
claiming an acyclicity it does not have, and `check-spec-counts.py` fails on it
by design. `render-spec-counts.py` deliberately will not paper over it: a cycle
between two packages is a paragraph to write, not a word to swap.

**Name resolution prefers a definition in the same file.** 129 of 317 macro
...
8 files remain blocked, on three prefixes, and each needs an individual
...
| `rp` | `linregm.sas`, `linregm_jr.sas`, `linregm` | `rp` looks like a plot prefix by pattern, but these are linear-regression engines |
| `lg` | `logit`, `logit.cases`, `logitgr` | new on 2026-09-09 with the picker fix; all three are extensionless, and `lg` had no blocked file while the scan could not see them |

⚠️ **`linregm` and `linregm.sas` are both blocked, and they are not the same
file** — 38 lines against 99. Whatever call `rp` gets has to be made about a
pair, not a file; see the union rule rather than picking one as canonical.

`mm`, `gm` and `nm` remain owner-undetermined in the canonicalization spec. They
block nothing today, so no decision is forced.

## Corpus-only files (178)
