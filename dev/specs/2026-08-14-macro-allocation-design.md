...
🔴 **ERRATUM 2026-09-09 — the scan reads 176 of the library's 281 files.** Its
`*.sas` glob never opened the 105 extensionless files, so `Total = 176`,
`Corpus-only = 73` and every per-package list below are computed over 63% of the
library. `kaplan`, validated by the call-site scan as a shared house primitive,
is one of the files it cannot see. Counts are held pending a re-run — see
