#!/usr/bin/env python3
"""Render roadmap tables from this package's inst/extdata/templates.json.

The catalog contains every template owed here. The generated section in the
roadmap is checked by check-roadmap-counts.py.
"""
import json
import os
import sys
...
def catalog_path():
    """The catalog ships in this package; absence is a packaging defect."""
    path = os.path.join(REPO, "inst", "extdata", "templates.json")
    if not os.path.isfile(path):
        raise SystemExit(f"Template catalog not found at {path}")
    return path


def load_catalog(all_rows=False):
    """Load every template row from this package's catalog."""
    path = catalog_path()
    with open(path, encoding="utf-8") as fh:
        d = json.load(fh)
    if "templates" not in d or not isinstance(d["templates"], list):
        raise SystemExit(f"{path} has no top-level 'templates' array")
    return d["templates"]
...
def render(rows, workflow_rows=None):
    ...
    if workflow_rows is None:
        workflow_rows = rows
    out = [BEGIN, "",
           "> Generated from this package's template catalog "
           "(`inst/extdata/templates.json`) by",
    ...
    out += ["## By workflow", "",
            "A workflow spans all template rows in this package. A member is "
            "complete when its template is shipped, being revisited, or in flight.", ""]
    wf = {}
    ...
    for name in sorted(wf):
        ...
        member_labels = [f"`{_label(r)}`" for r in members]
        if not outstanding:
            status_line = "**Complete.**"
        else:
            status_line = "Outstanding: " + ", ".join(
                f"`{_label(r)}`" for r in outstanding) + "."
