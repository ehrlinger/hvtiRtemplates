# The descriptives demo study: a synthetic cohort, the study around it, and
# the study choices each descriptive job is given.
#
# Sourced by descriptives-slides.qmd (the hands-on deck), descriptives-demo.R
# (everything in one run) and make-reference.R (the reference set). Keeping
# one builder means the slides, the one-shot run and the reference all
...
# The shapes give the reports something to find: heart failure falls over the
# years, LV mass drifts down, creatinine is missing for about 8%, and one
# patient in ten is lost to follow-up early, which the goodness-of-follow-up
# figure shows as blue points well below the diagonal. The seed is fixed, and
# R's generators give the same draws on every platform, so the reference
# numbers in reference/key-numbers.csv hold on any machine.
demo_data <- function() {
  set.seed(20260928)
  n <- 800L
  origin <- 1990
...
# ---- The study ---------------------------------------------------------------
# A study folder as study_setup() makes one, with the cohort registered as its
# built dataset. .rds keeps the variable labels, which the EDA pages and
# tables print; a .csv would drop them and every panel would be titled by its
# column name. Returns the study root.
demo_study <- function(root) {
  ...
  # study_setup() and register_data() print a study checklist; keep it quiet.
  ...
  saveRDS(demo_data(), file.path(study_dir("datasets", root), "built.rds"))
...
# ---- Each job's study choices ------------------------------------------------
# What a study author types under each job's EDIT: markers, as whole-line
# replacements keyed by a pattern that must match exactly one line. The demo
...
# line, so a template change that moves a choice stops the demo instead of
# rendering the default silently. Resolving the markers is what lets
# render_job(final = TRUE) accept the job; by hand, you delete each marker as
# you settle it.
...
# Scaffold one demo job by its "<prefix>-<qualifier>" name and apply its choices.
demo_job <- function(root, name) {
  parts <- strsplit(name, "-", fixed = TRUE)[[1L]]
  job <- add_job(parts[[1L]], "cohort", "demo", dir = root, qualifier = parts[[2L]])
  set_choices(job, demo_choices[[name]])
}

# Every job, scaffolded, set and rendered in the order of the slides.
# Returns the rendered reports, named by job.
demo_jobs <- function(root, quiet = TRUE) {
  jobs <- vapply(names(demo_choices), function(name) demo_job(root, name), character(1L))
  for (name in names(jobs)) {
    message("Rendering ", name, " ...")
    render_job(jobs[[name]], final = TRUE, quiet = quiet)
  }
