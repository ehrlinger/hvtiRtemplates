...
other prefixes remain unqualified. It is `NULL` and the column is `NA`
for a prefix with a single template. Naming no qualifier where a prefix
carries several is an error listing the choices, never a silent pick of
the first.
...
datasets with `register_data()`. The tutorial [Migrate a legacy study to
R
jobs](https://ehrlinger.github.io/hvtiRtemplates/articles/legacy-study-migration.md)
walks through adoption, study and named-subset registration, migration,
and output review using synthetic data.
...
`hv_check_docx()`. It writes beneath `documents/<endpoint>-<type>/` and
